...
class VideoPlayer:
    ...
    def update(self):
        # Use a separate thread for frame processing to avoid UI freezing
    ...
    def process_frame(self):
        if self.vid:
            ...
            if ret:
                # Perform GPU-accelerated processing here, e.g., using cupy
                frame_gpu = cp.asarray(frame)
                frame_processed = cp.asnumpy(self.process_on_gpu(frame_gpu))

                # Resize and display the frame
                frame_resized = self.resize_frame(frame_processed, 1600, 900)
                self.photo = ImageTk.PhotoImage(image=Image.fromarray(frame_resized))
                self.canvas.create_image(0, 0, image=self.photo, anchor=tk.NW)

    def process_on_gpu(self, frame_gpu):
        # Example GPU-accelerated processing (can be modified as needed)
        # Here, just returning the frame as-is for demonstration
        return frame_gpu
