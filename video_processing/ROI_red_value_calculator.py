...
class VideoFrameGrabber:
    ...
    def calculate_average_red_value(self):
        if self.vid is not None and self.roi_start is not None and self.roi_end is not None:
            ...
            while ret:
                red_channel = frame[int(self.roi_start[1]):int(self.roi_end[1]), int(self.roi_start[0]):int(self.roi_end[0]), 2]
            ...
            plt.title(f"{self.basename_without_ext} ROI Average Red Value over Time")
            plt.show()
            data_array = np.array(self.roi_values)
            save_path = f"{self.basename_without_ext}_red_event.npy"
